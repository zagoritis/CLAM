...
class GatedLinearCrossAttention(nn.Module):
    def __init__(self, d_model, nhead, gate_low_rank_dim=16, expand_k=0.5, gate_state=True, use_scan=False, **kwargs):
        ...
        if gate_state:
            self.gk_proj = nn.Sequential(nn.Linear(d_model, gate_low_rank_dim, bias=False), nn.Linear(gate_low_rank_dim, qk_dim, bias=True))
        else:
    ...
    def forward(self, query, key, value, *args, **kwargs):
        ...
        q, k, v, gk = (rearrange(x, 'b l (h d) -> b h l d', h=self.num_heads) for x in (q, k, v, gk))
...
class GLAMLP(nn.Module):
    def __init__(self, hidden_size: int, hidden_ratio: Optional[int] = None, intermediate_size: Optional[int] = None, hidden_act: str = 'swish'):
        super().__init__()
...
class CLAM(nn.Module):
    def __init__(self, cfg: Config):
        super().__init__()
        decoder_layer = GLADecoderLayer(d_model=cfg.MODEL.D_MODEL, nhead=cfg.MODEL.N_HEADS, dropout=cfg.MODEL.DROP_DEC, gate_state=cfg.CLUSTERING.GATE_STATE, expand_k=cfg.CLUSTERING.EXPAND_K, use_scan=cfg.CLUSTERING.USE_SCAN)
    ...
    def forward(self, memory, tgt_mask=None, memory_padding_mask=None, tgt_padding_mask=None, memory_pos=None):
        B = memory.size(0)
        query = self.query_embed.weight.unsqueeze(0).expand(B, -1, -1)
        tgt = torch.zeros_like(query)

        out = self._forward(tgt, memory, tgt_mask, query_pos=query, memory_key_padding_mask=memory_padding_mask, tgt_key_padding_mask=tgt_padding_mask, pos=memory_pos)
