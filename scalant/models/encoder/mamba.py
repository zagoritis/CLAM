...
class MAMBA(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.layers = nn.ModuleList([create_block(cfg.MODEL.D_MODEL, d_intermediate=0, ssm_cfg=mamba_cfg, rms_norm=True, fused_add_norm=True, layer_idx=i) for i in range(cfg.MODEL.N_LAYER)])
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if not self.return_intermediate:
            ...
            hidden_states = fused_add_norm_fn(hidden_states, self.norm_f.weight, self.norm_f.bias, eps=self.norm_f.eps, residual=residual, prenorm=False)
            return hidden_states

        return [fused_add_norm_fn(hidden_states, self.norm_f.weight, self.norm_f.bias, eps=self.norm_f.eps, residual=residual, prenorm=False) for hidden_states, residual in all_res]
