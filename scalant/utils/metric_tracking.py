...
class MeanTopKRecallMeter(object):
    """adapted from RULSTM"""
    def __init__(self, name, string_format='{:.3f}', device='cuda', **kwargs):
        self.name = name
    ...
    def update(self, logits_labels_dict, n=1):
        ...
        tp = (scores.topk(5, dim=1).indices == labels.unsqueeze(1)).any(dim=1)
    ...
    def synchronize_between_processes(self):
        if not is_dist_avail_and_initialized():
            return
        tps_all = torch.tensor(self.tps, device=self.device)
        nums_all = torch.tensor(self.nums, device=self.device)
        dist.barrier()
    ...
    def value(self, idx: Optional[int] = -1):
    ...
    def to_string(self, idx: Optional[int] = -1):
        return self.string_format.format(self.value(idx))


class MeanClassMeter:
    def __init__(self, name, string_format='{:.3f}', device='cuda', **kwargs):
        self.name = name
        self.string_format = string_format
        self.device = device
        ...
        self.num_classes = kwargs["num_classes"]
        self.eval_perc = kwargs.get("eval_perc", [1])

    def reset(self):
        self.tps = np.zeros((len(self.eval_perc), self.num_classes))
        self.nums = np.zeros((len(self.eval_perc), self.num_classes))

    def update(self, nums_tps_dict: Dict, n=1):
        del n  # Not used here.
        nums = nums_tps_dict["nums"]
        tps = nums_tps_dict["tps"]
        self.nums += nums
        self.tps += tps
    ...
    def value(self, idx: Optional[int] = -1):
        def calculate_acc(idx):
            tps = self.tps[idx, self.nums[idx] > 0]
            nums = self.nums[idx, self.nums[idx] > 0]
            acc = tps / nums
            return acc.mean() * 100 if len(acc) > 0 else None
        if isinstance(idx, int):
            return calculate_acc(idx)
        elif idx == "all":
            return [calculate_acc(i) for i in range(len(self.eval_perc))]
        else:
            raise ValueError(f"Index {idx} is not valid.")

    def to_string(self, idx: Optional[int] = -1):
        # print("nums: ", self.nums[-1])
        # print("total nums: ", self.nums[-1].sum())
        res = self.value(idx)
        if isinstance(res, list):
            return " ".join([self.string_format.format(num) for num in res])
        return self.string_format.format(res)
